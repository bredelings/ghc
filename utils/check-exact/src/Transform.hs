...
captureOrder' :: [GHC.LocatedA b] -> GHC.AnnSortKey
captureOrder' ls = GHC.AnnSortKey $ map (rs . GHC.getLocA) ls
...
captureLineSpacing :: [GHC.LHsDecl GHC.GhcPs] -> [GHC.LHsDecl GHC.GhcPs]
captureLineSpacing [] = []
captureLineSpacing [d] = [d]
captureLineSpacing (d1:d2:ds) = d1:captureLineSpacing (d2':ds)
  where
    (l1,_) = ss2pos $ rs $ GHC.getLocA d1
    (l2,_) = ss2pos $ rs $ GHC.getLocA d2
    d2' = setEntryDP' d2 (GHC.DP (l2-l1,0))
...
replaceDeclsValbinds (GHC.HsValBinds _ _b) new
    = do
        ...
        return (GHC.HsValBinds GHC.noAnn (GHC.ValBinds sortKey decs sigs))
...
replaceDeclsValbinds (GHC.EmptyLocalBinds _) new
    = do
        ...
        let decs = GHC.listToBag $ newBinds
        let sigs = newSigs
        let sortKey = captureOrder' new
        return (GHC.HsValBinds GHC.noAnn (GHC.ValBinds sortKey decs sigs))
